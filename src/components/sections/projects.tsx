          ...
          >
            ...
            <div className="absolute w-full h-1/2 bottom-0 left-0 bg-gradient-to-t from-black via-black/85 to-transparent pointer-events-none">
              <div className="flex flex-col h-full items-start justify-end p-6">
                ...
                <div className="text-xs bg-white text-black rounded-lg w-fit px-2 mb-2">
                  {project.category}
                </div>
                <div className="flex flex-wrap gap-2 mt-1">
                  {[...(project.skills.frontend || []), ...(project.skills.backend || [])]
                    .slice(0, 6) // Limit to 6 icons to prevent overcrowding
                    .map((skill, idx) => (
                      <div key={idx} className="bg-white/20 p-1 rounded-full text-white text-sm backdrop-blur-sm" title={skill.title}>
                        {skill.icon}
                      </div>
                    ))}
                </div>
