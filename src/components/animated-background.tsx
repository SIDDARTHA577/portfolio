...
const AnimatedBackground = () => {
  ...
  // --- Helper for Mapping ---
  // Map skill names to 3D object names if they differ
  const skillToObjectMapper: Record<string, string> = {
  ...
  const getMappedName = (name: string) => {
    ...
    return skillToObjectMapper[lower] || lower;
  };

  const findSkillByObjectName = (objName: string): Skill | undefined => {
    // 1. Try direct match
    if (SKILLS[objName as SkillNames]) return SKILLS[objName as SkillNames];

    // 2. Try to find a skill where the mapped name matches the object name
    const foundSkill = Object.values(SKILLS).find(skill => {
      return getMappedName(skill.name) === objName || skill.name === objName;
    });
    return foundSkill;
  };
  ...
  const handleMouseHover = (e: SplineEvent) => {
    ...
    if (e.target.name === "body" || e.target.name === "platform") {
      ...
      const skill = findSkillByObjectName(e.target.name);
      if (skill && (!selectedSkillRef.current || selectedSkillRef.current.id !== skill.id)) {
        if (selectedSkillRef.current) playReleaseSound();
        playPressSound();
  ...
  const getKeycapsAnimation = () => {
    ...
    const start = () => {
      ...
      Object.values(SKILLS)
        ...
        .forEach((skill, idx) => {
          // FIX: Use filtered/mapped name to find object
          let keycap = splineApp.findObjectByName(skill.name);
          if (!keycap) {
            keycap = splineApp.findObjectByName(getMappedName(skill.name));
          }
  ...
  const updateKeyboardTransform = async () => {
    ...
    const keycaps = allObjects.filter((obj) => obj.name === "keycap" || obj.name === "Keycap");
    ...
    // SAFELY animate generic keycaps without risking permanent hiding
    keycaps.forEach(async (keycap, idx) => {
      // Force visible first
      keycap.visible = true;

      // Optional gentle pop-in that doesn't rely on being hidden
      gsap.from(
        keycap.position,
        { y: 200, duration: 0.5, delay: idx * 0.05, ease: "bounce.out" }
      );
    });
  };
  ...
  useEffect(() => {
    ...
    const manageAnimations = async () => {
      ...
      // FIX: Enable for SKILLS as well, so user sees animation
      if (activeSection === "contact" || activeSection === "skills") {
        // Only do teardown flip in contact, logic inspired by Reference but modified for Skills support
